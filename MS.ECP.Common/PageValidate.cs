...
namespace MS.ECP.Common
{
    ...
    public class PageValidate
    {
        ...
        private static Regex RegPhone2 = new Regex("((\\d{11})|^((\\d{7,8})|(\\d{4}|\\d{3})-(\\d{7,8})|(\\d{4}|\\d{3})-(\\d{7,8})-(\\d{4}|\\d{3}|\\d{2}|\\d{1})|(\\d{7,8})-(\\d{4}|\\d{3}|\\d{2}|\\d{1}))$)");
        ...
        private static Regex RegEmail2 = new Regex(@"^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$");
        ...
        {
            Match m = RegEmail2.Match(inputData.ToLower());
            return m.Success;
        }
